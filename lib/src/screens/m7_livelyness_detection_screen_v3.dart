...
import 'package:livelyness_detection/src/core/helpers/face_scan.dart';
import '../core/enums/face_status.dart';

class LivelynessDetectionPageV3 extends StatelessWidget {
  final DetectionConfig config;

  const LivelynessDetectionPageV3({
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: SafeArea(
        child: LivelynessDetectionScreenV3(
          config: config,
        ),
      ),
    );
...
class LivelynessDetectionScreenV3 extends StatefulWidget {
  final DetectionConfig config;

  const LivelynessDetectionScreenV3({
    required this.config,
    super.key,
  });

  @override
  State<LivelynessDetectionScreenV3> createState() =>
      _LivelynessDetectionScreenAndroidState();
}

class _LivelynessDetectionScreenAndroidState
    extends State<LivelynessDetectionScreenV3> {
  ...
  final currentCamDescVN = ValueNotifier<CameraDescription?>(null);
  ...
  Future<void> _initializeCamera() async {
    ...
    currentCamDescVN.value = _cameras.firstWhere(
      (final e) => e.lensDirection == CameraLensDirection.front,
      orElse: () => _cameras.first,
    );
    _cameraController = CameraController(
      currentCamDescVN.value!,
    ...
    // Start real-time image stream analysis
    _cameraController!.startImageStream((CameraImage image) {
      if (!_isDetecting) {
        _isDetecting = true;
        _processCameraImage(image).then((_) => _isDetecting = false);
      }
    });
  ...
  Future<void> _processCameraImage(CameraImage img) async {
    ...
    try {
      ...
      await _processImage([detectedFaces]);
      if (mounted) {
        setState(
          () => _isProcessing = false,
        );
  ...
  Future<void> _completeStep({
    ...
    if (step != lastCompletedStep) {
      ...
      imgPaths.add(clickedImage?.path);
    }
